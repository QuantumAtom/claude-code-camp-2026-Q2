...
### File-system / shell tools — now MCP-backed

`Tools::FileSystem` and `Tools::Shell` (tool names, parameters, and behavior
all unchanged from step 10) now run inside separate
[MCP](https://modelcontextprotocol.io) server subprocesses —
`lib/boukensha/mcp_servers/file_system_server.rb` and `shell_server.rb` —
spawned over stdio and bridged into the `Registry` via
`Boukensha::McpClient`. Only the transport changed: local closures became a
real client/server boundary talking JSON-RPC, matching the same MCP
architecture the Python port already uses. `Tools::Mud` is unaffected and
still runs in-process.

Requires the `mcp` gem (added to the `Gemfile`/gemspec, `~> 1.0`).
