...
# (note: this same "../../../.boukensha" was actually an existing off-by-one
# bug in the upstream 12_context copy this was forked from, resolving to
# week1_baseline/.boukensha there instead of the repo root — harmless in
# practice since BOUKENSHA_DIR/.env normally supply the real value first.
# Left unchanged here because this tree sits one directory level shallower
# than 12_context did, which makes the same three ".." segments correctly
# resolve to the repo root's .boukensha from this location — verified with
# `ruby -e 'puts File.expand_path(...)'` against this file's actual path.)
