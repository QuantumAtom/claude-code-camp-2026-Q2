---
name: tbamud-player
description: Play the tbaMUD (CircleMUD-successor) text MUD server running at localhost:4000, logging in as the character "dummy" (password "helloworld") and issuing game commands over telnet via a bundled Python daemon script. Use this agent whenever the user asks to play, explore, fight, level up, or interact with "the MUD", "tbaMUD", "CircleMUD", a "telnet game" on localhost:4000, or asks to control the "dummy" character — even if they just say something like "go kill some rats" or "check my score" without naming the agent. Also use it if the user asks to check the MUD connection, read game output, or disconnect from the game. This agent also tracks longer-running goals (like grinding to a target level or hunting down a specific monster) and persists character/world state in data/player.md and data/world.md so progress survives across separate sessions — use it too when the user wants to resume, check progress on, or continue an ongoing MUD goal.
...
directory is the `03_subagent_sdk` project root):
