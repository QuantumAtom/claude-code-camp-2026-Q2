#!/usr/bin/env python3
"""Run a tbaMUD player-journey orchestrator using the Claude Agent SDK.

Earlier iterations of this exploration (03a/03b) relied on Claude Code's
filesystem convention: dropping a frontmatter file at
.claude/agents/tbamud-player.md and letting the CLI auto-discover it. This
script instead defines the same subagent programmatically via
AgentDefinition and registers it on ClaudeAgentOptions, so the SDK process
never depends on .claude/agents/*.md being present.

The subagent's own support files (the daemon script and command cheat
sheet under .claude/agents/tbamud-player/) are unrelated to that discovery
mechanism -- they're just files the subagent reads/executes via its Bash
tool -- so they stay where they are.
"""
...
TBAMUD_PLAYER_DESCRIPTION = (
    "Play the tbaMUD (CircleMUD-successor) text MUD server running at "
    "localhost:4000, logging in as the character \"dummy\" (password "
    "\"helloworld\") and issuing game commands over telnet via a bundled "
    "Python daemon script. Use this agent whenever the user asks to play, "
    "explore, fight, level up, or interact with \"the MUD\", \"tbaMUD\", "
    "\"CircleMUD\", a \"telnet game\" on localhost:4000, or asks to control "
    "the \"dummy\" character -- even if they just say something like \"go "
    "kill some rats\" or \"check my score\" without naming the agent. Also "
    "use it if the user asks to check the MUD connection, read game "
    "output, or disconnect from the game. This agent also tracks "
    "longer-running goals (like grinding to a target level or hunting "
    "down a specific monster) and persists character/world state in "
    "data/player.md and data/world.md so progress survives across "
    "separate sessions -- use it too when the user wants to resume, check "
    "progress on, or continue an ongoing MUD goal."
)
...
(a monster attacks, a door is locked, you starve). A bundled daemon
wraps that connection so the session survives across multiple tool
calls, the same way a human would keep a telnet client open in a
terminal tab while they play.

## The tool

`.claude/agents/tbamud-player/scripts/mud_client.py` manages the whole
lifecycle. Run it with `python3` (paths below assume your working
directory is the `03b_subagent_sdk` project root):

```
python3 .claude/agents/tbamud-player/scripts/mud_client.py start                # connect + log in as dummy/helloworld
python3 .claude/agents/tbamud-player/scripts/mud_client.py send "look"           # send one command, prints the response
python3 .claude/agents/tbamud-player/scripts/mud_client.py send "north" --wait 1 # give slow responses more time before reading
python3 .claude/agents/tbamud-player/scripts/mud_client.py read                  # print any new output that arrived on its own
python3 .claude/agents/tbamud-player/scripts/mud_client.py status               # is it connected? show recent output
python3 .claude/agents/tbamud-player/scripts/mud_client.py stop                  # disconnect cleanly
```
...
control socket) lives under `.claude/agents/tbamud-player/session/`
next to the script, not `/tmp`, so you can read
`.claude/agents/tbamud-player/session/transcript.log` any time to see
the full history if something seems off.
...
fit in a single conversation. Two files at the project root, `data/player.md`
and `data/world.md`, exist so that kind of goal survives across
separate sessions or automated loop passes, the same way a human
player keeps notes between sessions instead of re-learning the map and
forgetting what they were working toward.

- **`data/player.md`** is the character's record: level/HP/mana/move at
...
- **`data/world.md`** is the shared map: rooms visited, their exits, NPCs
...
`.claude/agents/tbamud-player/references/commands.md`. When in doubt,
`help` (and `help <topic>`) is always available in-game and is more
authoritative than the cheat sheet for anything server-specific.
...
game printed, `.claude/agents/tbamud-player/session/transcript.log`
has the untouched record.
"""
...
ORCHESTRATOR_SYSTEM_PROMPT = (
    "You are a player-journey orchestrator. The user will ask you to play, "
    "explore, or check progress on a tbaMUD session running at "
    "localhost:4000. Delegate that work to the tbamud-player agent via the "
    "Agent tool rather than driving the MUD yourself."
)
...
def build_options() -> ClaudeAgentOptions:
    return ClaudeAgentOptions(
        ...
        allowed_tools=["Agent"],
        # SDK isolation mode: don't fall back to any filesystem-discovered
        # settings/subagents (e.g. a stray .claude/agents/*.md) -- the
        # tbamud-player agent above is the only source of truth.
        setting_sources=[],
...
def print_message(message: Message) -> None:
    if isinstance(message, AssistantMessage):
        ...
        print(f"\n--- done in {message.duration_ms}ms, {message.num_turns} turns, cost {cost} ---")


async def main() -> None:
    # ClaudeSDKClient (rather than the stateless query()) keeps one CLI
    # process and conversation alive across turns, so the tbamud-player
    # subagent's own state (its MUD daemon connection) and the
    # orchestrator's conversation history both persist between messages.
    async with ClaudeSDKClient(options=build_options()) as client:
        ...
        print("\nEntered interactive mode -- type a message, or 'exit'/Ctrl-D to quit.")
        while True:
            try:
